...
/**
 * Exchange an authenticated Firebase user for a ChatView session via the
 * backend ``/auth/google/firebase/`` endpoint.
 */
 ...
 */
export async function signInWithGoogleAndExchange(): Promise<GoogleExchangeResult> {
  ...
  return exchangeFirebaseUser(result);
}

/**
 * Exchange a Google ID token obtained from Google Identity Services (One Tap)
 * for a ChatView session. Signs into Firebase with the Google credential and
 * reuses the same backend exchange as the popup flow.
 */
